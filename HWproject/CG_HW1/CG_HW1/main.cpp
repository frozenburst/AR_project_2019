...
bool project_otho;
int mode;	// 1:translation 2:rotation 3:scaling 4:center 5:eye
...
// [Done] compute persepective(perspective) projection matrix
void setPerspective()
{
	/*
...
void onKeyboard(unsigned char key, int x, int y)
{
	...
	case 'c':
		...
		break;
	case 'i':
		printf("Model's position:(%.4f,%.4f,%.4f), rotation:(%.4f,%.4f,%.4f), scaling:(%.4f,%.4f,%.4f)\n"
		...
		printf("Camera's position:(%.4f,%.4f,%.4f), center:(%.4f,%.4f,%.4f)\n, "
			, main_camera.position.x, main_camera.position.y, main_camera.position.y
			, main_camera.center.x, main_camera.center.y, main_camera.center.z);
		if (project_otho)
			printf("Projection type: Orthogonal, ");
		else
		{
			printf("Projection type: Perspective, ");
		}
		printf("with left:%f, right:%f, top:%f, bottom:%f, nearClip:%f, farClip:%f, fovy:%f.\n", proj.left, proj.right, proj.top, proj.bottom, proj.nearClip, proj.farClip, proj.fovy);
		break;
	case 't':
		mode = 1;
		printf("set to Geometrical transformation : translation.\n");
		break;
	case 's':
		mode = 3;
		printf("set to Geometrical transformation : scaling.\n");
		break;
	case 'r':
		mode = 2;
		printf("set to Geometrical transformation : rotation.\n");
		break;
	}
	printf("\n");
}
